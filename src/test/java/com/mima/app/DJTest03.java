...
public class DJTest03 {
	...
	public void testDJ03() {
		...
		Dijkstra dj = new Dijkstra(new StrasseBoImpl(str), new PointBoImpl(new StrassenMock03()), b);
		try {
			opb = dj.run(new Long(1), new Long(4));
			assertTrue(true);
			System.out.println(opb.getStreckenInklGewichtung());
			assertEquals(19, opb.getStreckenInklGewichtung(), 0);
			assertEquals("A, C, D", opb.getHistory());
		} catch (Exception e) {
			e.printStackTrace();
			assertTrue(false);
		}
